...
class assign_feedback_file extends assign_feedback_plugin {
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        global $DB;
        $lastupload = $DB->get_record('assign_vimeo',array('assign'=>$grade->assignment,'author'=>$DB->get_record('user',array('id'=>$grade->grader))->username,'studentid'=>$userid));
    ...
    public function update_file_count($grade) {
        ...
        foreach ($files as $file) { 
            ...
            $nomearquivo = 'Correção_atividade_'.$grade->assignment.'_'.$COURSE->shortname.'_'.$USER->firstname.$USER->lastname;
            ...
            $videorecord->assign =  $grade->assignment;
            $videorecord->author = $USER->username;
            $videorecord->link = $uri;  
            $videorecord->embed = $response['body']['embed']['html'];
            $videorecord->studentid = $grade->userid;       
        }
        if ($filefeedback) {
            $filefeedback->numfiles = $this->count_files($grade->id, ASSIGNFEEDBACK_FILE_FILEAREA);
            $oldvideo = $DB->get_record('assign_vimeo', array('assign'=>$grade->assignment,'author'=>$USER->username,'studentid'=>$grade->userid));
            //var_dump($oldvideo);
            if(!$oldvideo){
    ...
    public function view_summary(stdClass $grade, & $showviewlink) {
        ...
        if(isset($grade->grader) && $grade->grader > 0){
            $idSearch = $grade->grader;
        }else{
            $idSearch = $USER->id;
        }
        /*echo '<pre>';
        print_r($grade->grader);
        echo '<br>';
        print_r($grade->assignment);
        echo '<br>';
        print_r($grade->userid);
        exit(0);*/  
        ...
        if ($count <= ASSIGNFEEDBACK_FILE_MAXSUMMARYFILES) {
            $uservimeo = $DB->get_record('user',array('id'=>$idSearch));                                            
            $embed = $DB->get_record('assign_vimeo', array('assign'=>$grade->assignment,'author'=>$uservimeo->username,'studentid'=>$grade->userid));
            if($embed){
                if(isset($_GET['action']) && $_GET['action'] == 'grading'){
